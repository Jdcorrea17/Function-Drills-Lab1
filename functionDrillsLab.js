...
const exclaimThree = str => `${str.toUpperCase(exclaimThree)} !!!`
...
console.log(bestMovie('Fast and Furious'))
...
function theEliminator(contestant, loser){
  for(let i = 0; i < contestant.length; i++){
    if(contestant[i] === loser){
      contestant.splice(i, i)
      return contestant
    }
  }
}

let newContestant = theEliminator(contestants, loser)
...
let newEmail = emailCheck('jdcorrea17@gmail.com')
...
function chocolatebuy(gold){
  return gold % 3
}

let totalFrogs = chocolatebuy(12)
...
let totalFrogs2 = chocolatebuy(4)
